...
#+BEGIN_SRC sh :eval no :tangle no
  ...
  REPOS="github/martinhomuth/repos/emacs-org git/dotfiles git/org"

  echo -n "Updating repositories"
  for repo in $REPOS; do
      cd $HOME/$repo
      echo -n "."
      echo "$(date) - $repo" >> /var/log/startup.log
      git pull &>>/var/log/startup.log
      if [ $? -eq 1 ]; then
    ...
    (use-package org-caldav
      ...
      (setq org-caldav-calendars
            '((:calendar-id "personal"
                            :files ("~/git/org/personal.org" "~/git/org/organizer.org" "~/git/org/journal.org")
