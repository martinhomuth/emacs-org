...
Emacs-solo showed me some performance hacks that seems to be valuable
...
I am working with different computers on the same text base controlled by git and some background jobs that keep the git
repositories current by some means. As the files are always opened and essential to my organization, I need to make sure
that the opened buffers are current all the time. global-auto-revert-mode does the trick, but we need to make sure not
to bother remote files too much slowing down tramp.
...
I found showing line numbers on the side to be rather helpful in general so I will try to enable them all the time.
...
#+RESULTS:
| display-line-numbers-mode | text-mode-hook-identify |

Elisp overlay evaluation appears to be a rather nice feature.
...
For some reason I wanted to try scrolling by page and recenter the cursor. Now lets try this with smoother scrolling.
...
Moving by sentence does expect a sentence to end on two spaces. As I am no writer and this is nothing I really encounter
in real life, let's adjust this
...
It is fairly often required to calculate the hash of a file to make sure it is the right one and get that hash into the
clipboard. So we create a function that calculates that hash, prints it to the *Message* buffer and copies it to the kill
ring.
...
#+RESULTS:
: mh/dired-copy-sha256

* Tramp

Apparantly Tramp can be made fairly responsive again as noted in the [[https://www.gnu.org/software/tramp/#Frequently-Asked-Questions-1][FAQ]].
...
After realizing that auto-complete is dead lets switch to company.
...
When editing bitbake recipes it is very often the case that a variable contains many elements. Best practice is to fill
such a variable line by line to make revision control easier. The following function transforms a string into multiple
lines.
...
#+RESULTS:
| #[nil ((mapc (lambda (pair) (push pair prettify-symbols-alist)) '(([ ] . ☐) ([X] . ☑) ([-] . ❍))) (prettify-symbols-mode)) nil] | er/add-org-mode-expansions | #[0 \301\211\207 [imenu-create-index-function org-imenu-get-tree] 2] | turn-on-auto-fill | #[nil ((mapc #'(lambda (pair) (setq prettify-symbols-alist (cons pair prettify-symbols-alist))) '(([ ] . ☐) ([X] . ☑) ([-] . ❍))) (prettify-symbols-mode)) nil] | org-fancy-priorities-mode | org-bullets-mode | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-fold-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes |
...
#+RESULTS:
| t | Tasks | entry | (file+headline ~/git/org-files/personal.org INBOX) | * TODO %^{Task} %^G |
...
** Org-GTD
:PROPERTIES:
:ID:       Org-GTD-2025-11-28-09-46-34
:END:
...
*** Provide =mh-go.el= module

#+begin_src emacs-lisp :tangle "mh-modules/mh-go.el"
  (provide 'mh-go)
#+end_src
...
Some adjustments to prog-mode in general.
...
Some adjustments to the existing mode have to be performed.
...
#+RESULTS:
| (lambda nil (define-key c-mode-base-map (kbd C-c C-l) 'compile)) | (lambda nil (c-add-style linux-tabs-only '(linux (c-offsets-alist (arglist-cont-nonempty c-lineup-gcc-asm-reg c-lineup-arglist-tabs-only))))) | my/general-c-mode-configuration | my/add-semantic-to-autocomplete | ac-cc-mode-setup |
