...
(defun mh/eval-last-sexp-overlay (arg)
    ...
    (if arg
      ...
      (let* ((value (elisp--eval-last-sexp nil))
        ...
        (run-with-timer
         ...
         ov)))))
(global-set-key (kbd "C-x C-e") #'mh/eval-last-sexp-overlay)
...
(defun mh__add-leading-zero(value)
  "Adds a leading zero if the length of VALUE is 1"
  (let ((ret value))
    (when (length= ret 1)
      (setq ret (concat "0" ret)))
      ret)
  )
